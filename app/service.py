...
class OzonParser():
    ...
    def parse_html(html):
        ...
        try:
            ...
            block_descr = json.loads(soup.find(id=re.compile('^state-webCharacteristics')).get('data-state')) # вытаскивание блока с описанием
            product_name = block_descr['productTitle'].split('Характеристики: ')[1]
            main_category = block_descr['characteristics'][0]['short'][0]['values'][0]['text']

            block_tags = json.loads(soup.find(id=re.compile('^state-breadCrumbs')).get('data-state')) # вытаскивание блока с описанием
            tags = []
            for item in block_tags['breadcrumbs']:
                tags.append((item["text"]))

            block_id = json.loads(soup.find(id=re.compile('^state-webDetailSKU')).get('data-state')) # вытаскивание блока с описанием
            product_id = block_id['sku']
